...
def train(sess):
    ...
    conv_aux3, conv_aux2, conv_aux1, y_conv = Net(x)


    y_ = tf.placeholder(tf.float32,shape=(ParSet['BATCH_SIZE'],None,None,None,1),name='y_input')
    y_aux3 = tf.placeholder(tf.float32, shape=(ParSet['BATCH_SIZE'], None, None, None, 1), name='y_aux3')
    y_aux2 = tf.placeholder(tf.float32, shape=(ParSet['BATCH_SIZE'], None, None, None, 1), name='y_aux2')
    y_aux1 = tf.placeholder(tf.float32, shape=(ParSet['BATCH_SIZE'], None, None, None, 1), name='y_aux1')

    with tf.name_scope('dice'):

        loss_aux3 = cross_entropy(y_aux3,conv_aux3)
        loss_aux2 = cross_entropy(y_aux2,conv_aux2)
        loss_aux1 = cross_entropy(y_aux1,conv_aux1)
        loss_main = cross_entropy(y_,y_conv)

        loss_total = loss_main + loss_aux1 + loss_aux2 +loss_aux3
    ...
    def feed_dict():
        ...
        image, label, label_aux1, label_aux2, label_aux3 = data_load_train(nii_index=np.random.choice(data_rang))

        return {x:image , y_:label, y_aux1:label_aux1, y_aux2:label_aux2, y_aux3:label_aux3}
